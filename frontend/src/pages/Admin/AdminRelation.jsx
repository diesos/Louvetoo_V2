  ...
  useEffect(() => {
    const fetchUsersAndChildren = async () => {
      try {
        ...
        const childrenResponse = await axios.get('http://localhost:3000/api/children/getallchildren');

        // Debugging output
        console.log('Users response:', usersResponse.data);
        console.log('Children response:', childrenResponse.data);

        // Assuming the structure of the response is similar to children response shown
        ...
        <div>
          ...
          <select value={userId} onChange={(e) => setUserId(e.target.value)}>
            ...
            {Array.isArray(users) && users.map(user => (
              <option key={user.id} value={user.id}>{`${user.prenom} ${user.nom} (ID: ${user.id})`}</option>
            ))}
          </select>
        </div>
        <div>
          ...
          <select value={enfantId} onChange={(e) => setEnfantId(e.target.value)}>
            ...
            {Array.isArray(children) && children.map(child => (
              <option key={child.id} value={child.id}>{`${child.prenom} ${child.nom} (ID: ${child.id})`}</option>
            ))}
          </select>
        </div>
