...
const AdminUsers = () => {
    ...
    const handleChange = async (e) => {
        ...
        if (name === 'firstname' || name === 'lastname') {
            try {
                const response = await axios.get(`http://localhost:3000/api/users/search?prenom=${value}`);
    ...
    return (
        <>
            ...
            <h1 style={{textAlign: 'center', margin: '15px'}}>
                {currentAction === 'edit' ? "Modifier un Utilisateur" :
                currentAction === 'delete' ? "Supprimer un Utilisateur" :
                currentAction === 'add' ? "Ajouter un Utilisateur" :
                currentAction === 'getById' ? "Rechercher un Utilisateur" : ""}
            ...
            {currentAction === 'getAll' ? (
                <div style={{ textAlign: 'center' }}>
                    ...
                    <button onClick={() => setAllUsers([])} style={{margin:'10px'}}>Réinitialiser</button>

                    {allUsers.length > 0 && (
                        <div>
                            <h2 style={{margin: '15px'}}>Résultat total : {allUsers.length}</h2>
                ...
                <form onSubmit={currentAction === 'delete' ? handleDelete : currentAction === 'getById' ? fetchUserById : handleSubmit}>
                    ...
                    <button type="submit">
                        {currentAction === 'edit' ? "Modifier" :
                        currentAction === 'delete' ? "Supprimer" :
                        currentAction === 'add' ? "Ajouter" :
                        currentAction === 'getById' ? "Rechercher" : ""}
                    </button>
                </form>
            )}
